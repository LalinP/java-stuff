package com.java.stuff.chess.pieces;
...
public class Piece {
	 ...
	 * @param moveToXPosition The specified x location
	 * @param yPosition The specified y location
	 * @return
	 */
	protected boolean isMovingDiagonal(int moveToXPosition, int moveToYPosition) {
		int xStart = 0;
		int yStart = 0;
		int xFinish = 1;
		//int yFinish = 1;
		...
		int xTotal = Math.abs(moveToXPosition - this.getXLocation());
		int yTotal = Math.abs(moveToYPosition - this.getYLocation());
		
		if (xTotal == yTotal){ // moving along the X axis
			if (moveToXPosition < this.getXLocation()){
				xStart = moveToXPosition;
				xFinish = this.getXLocation();
			}
			else if (moveToXPosition > this.getXLocation()){
				xStart = this.getXLocation();
				xFinish = moveToXPosition;
			}
			else
				return false;
			
			if (moveToYPosition < this.getYLocation()){
				yStart = moveToYPosition;
				//yFinish = this.getYLocation();
			}
			else if (moveToYPosition > this.getYLocation()){
